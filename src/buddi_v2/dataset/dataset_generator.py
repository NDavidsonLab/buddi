...
def get_dataset(
    ...
    dtypes: Optional[Dict[str, np.dtype]] = {
        "label": np.int32,
        "stim": np.int32,
        "samp_type": np.int32,
    },
    **kwargs: Dict[str, np.ndarray],  # inputs
    ...
    dataset = tf.data.Dataset.from_generator(
        lambda: _dataset_generator(
            input_tuple_order, output_tuple_order, num_samples, **kwargs
        ),
        output_signature=_dataset_output_signature(
            input_tuple_order, output_tuple_order, dtypes, **kwargs
        ),
    ...
    **kwargs: Dict[str, np.ndarray],  # inputs
    ...
    output_signature = (
        tuple(
            ...
            for key in input_tuple_order
        ),
        tuple(
            tf.TensorSpec(shape=kwargs[key].shape[1:], dtype=dtypes[key])
            if key in kwargs
            else tf.TensorSpec(shape=(1,), dtype=dtypes[key])
            for key in output_tuple_order
        ),
    )
