...
def generate_pseudo_bulk_from_counts(
    ...
    if (
        cell_noise is None
    ):  # Generate default noise #TODO: this is quite counter-intuitive behavior for None
        cell_noise = [
            np.random.lognormal(0, 0.1, in_adata.shape[1]) for _ in range(num_celltypes)
        ]
        ...
        if len(cell_noise) == 0:
            ...
            raise ValueError(
                f"Length of cell_noise list ({len(cell_noise)}) does not match number of cell types ({num_celltypes})."
            )
        else:
            if not all(isinstance(noise, np.ndarray) for noise in cell_noise):
                raise ValueError(
                    "All elements in cell_noise list must be numpy arrays."
                )
            if not all(len(noise) == in_adata.shape[1] for noise in cell_noise):
                raise ValueError(
                    "All elements in cell_noise list must have the same length as the number of genes in in_adata."
                )
    elif isinstance(cell_noise, Tuple):
        if len(cell_noise) == 2 and all(
            isinstance(param, float) for param in cell_noise
        ):
            # Generate noise based on provided mean and std
            cell_noise = [
                np.random.lognormal(cell_noise[0], cell_noise[1], in_adata.shape[1])
                for _ in range(num_celltypes)
            ]
        else:
            raise ValueError(
                "If cell_noise is a tuple, it must contain two float values (mean, std)."
            )
    total_expr_list = []

    for samp_idx, (_, count_profile) in enumerate(
        tqdm(count_df.iterrows(), total=len(count_df))
    ):
...
def generate_pseudo_bulk_from_props(
    ...
    count_df = utils.generate_count_from_props(
        props_df, num_cells, random_num_cell_range
    )
