...
def fit_buddi(
    ...
    for epoch in range(epochs):
        ...
        if _ds_test_supervised is not None:
            ...
            for test_sup_batch in tqdm(
                _ds_test_supervised, desc="Testing supervised model"
            ):
                ...
                sup_test_batch_losses.append(
                    [accu.result().numpy() for _, accu in val_accumulators.items()]
                )
        ...
        if _ds_test_unsupervised is not None:
            ...
            for test_unsup_batch in tqdm(
                _ds_test_unsupervised, desc="Testing unsupervised model"
            ):
                test_unsup_x, test_unsup_y = test_unsup_batch
                test_unsup_loss = unsupervised_model.test_on_batch(
                    test_unsup_x, test_unsup_y
                )
                for name, value in zip(
                    unsupervised_model.metrics_names, test_unsup_loss
                ):
                    val_accumulators[name].update_state(value)
                unsup_test_batch_losses.append(
                    [accu.result().numpy() for _, accu in val_accumulators.items()]
                )
        ...
        if len(sup_test_batch_losses) > 0:
            ...
            test_sup_loss_df = pd.concat(
                [test_sup_loss_df, test_sup_batch_loss_df], ignore_index=True
            )
        if len(unsup_test_batch_losses) > 0:
            ...
            test_unsup_loss_df = pd.concat(
                [test_unsup_loss_df, test_unsup_batch_loss_df], ignore_index=True
            )
    ...
    if test_sup_loss_df.shape[0] > 0:
        test_sup_loss_df.columns = [
            col if i != 1 else "X_reconstruction_loss"
            for i, col in enumerate(test_sup_loss_df.columns)
        ]
    ...
    if test_unsup_loss_df.shape[0] > 0:
        test_unsup_loss_df.columns = [
            col if i != 1 else "X_reconstruction_loss"
            for i, col in enumerate(test_unsup_loss_df.columns)
        ]
