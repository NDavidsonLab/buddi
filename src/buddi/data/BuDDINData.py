...
class BuDDINData:
    ...
    def __init__(
        ...
        for field in encode_fields:
            ...
            self._encode_data[field] = pd.DataFrame(
                arr, columns=feat_names, index=meta.index
            )

        # 3) Define data
        self.data = {"X": X, "y": y, "meta": meta, **self._encoded_arrays}
    ...
    def query(self, **filters):
        ...
        for col_name, filter_values in filters.items():
            ...
            _values = (
                filter_values
                if isinstance(filter_values, (list, tuple, set, np.ndarray, pd.Series))
                else [filter_values]
            )
    ...
    def __subsample(
        ...
        for retrieval in retrievals:
            retrievals_sampled.append(
                retrieval[idx_sample, :].copy()
                if isinstance(retrieval, np.ndarray)
    ...
    def get(
        ...
        for key in keys:
            _data = self.data[key]
            returns.append(
                _data[self._selection, :].copy()
                if isinstance(_data, np.ndarray)
                else _data.iloc[self._selection, :].copy()
            )

        returns = self.__subsample(
            returns, n_samples=n_samples, replace=replace, random_state=random_state
        )
    ...
    def plot(
        ...
        **kwargs,
        ...
        if color_by is None:
            color_by = self._encode_fields + [self._split_column, self._ct_column]
        else:
    ...
    def __check_x(self, X: np.ndarray, gene_names: Optional[List[str]] = None) -> None:
        """
        ...
        if gene_names is not None:
            if len(gene_names) != X.shape[1]:
                raise ValueError(
                    "gene_names must have the same length as the number of columns in X"
                )
        return None

    def __check_y(
        self, y: np.ndarray, cell_type_names: Optional[List[str]] = None
    ) -> None:
        """
        Check if the input data is a numpy array.
        """
        ...
        if cell_type_names is not None:
            if len(cell_type_names) != y.shape[1]:
                raise ValueError(
                    "cell_type_names must have the same length as the number of columns in y"
                )
        return None
        ...
        if encode_fields is not None:
            for field in encode_fields:
                if field not in meta.columns:
                    raise ValueError(
                        f"encode_field '{field}' not found in meta columns"
                    )
