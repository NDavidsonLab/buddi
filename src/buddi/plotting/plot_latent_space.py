...
def _get_projection(z, type="PCA"):
    ...
    proj_df = pd.DataFrame(proj.fit_transform(z), columns=[f"{type}_0", f"{type}_1"])
...
def plot_latent_spaces_buddi4(
    ...
    color_legend_names = ["Cell Type", "Sample ID", "Perturbation", "Samp Type"]
    ...
    fig, axs = plt.subplots(
        ...
        else (panel_width * len(latent_spaces), panel_width * len(color_vecs)),
    )
    ...
    for i, (latent_space_name, latent_space) in enumerate(
        ...
        for j, (color_legend_name, color_vec) in enumerate(
            zip(color_legend_names, color_vecs)
        ):
            ...
            if (
                i != len(latent_spaces) - 1 or color_legend_name == "Sample ID"
            ):  # do not display legend for the sample ids
...
def plot_latent_spaces_buddi(
    ...
    fig, axs = plt.subplots(
        ...
        figsize=figsize
        if figsize is not None
        else (panel_width * n_latent_spaces, panel_width * n_colors),
    )
    ...
    for i, (latent_space_name, latent_space) in enumerate(z_mu_dict.items()):
        ...
        for j, (color_legend_name, color_vec) in enumerate(z_color_dict.items()):
            ...
            if (orig_leg is not None) and len(
                np.unique(color_vec)
            ) > _max_legend_categories:
                orig_leg.remove()
