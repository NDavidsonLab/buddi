...
def plot_loss(loss_df, panel_width=5, show_plot=True, save_path=None):
    """
    ...
    loss_cols = [col for col in loss_df.columns if col.endswith("_loss")]
    recon_loss_cols = [col for col in loss_cols if col.startswith("X_reconstruction_")]
    classifier_loss_cols = [col for col in loss_cols if col.startswith("classifier_")]
    ...
    loss_df["n_batches"] = (
        loss_df.groupby(["epoch", "split", "type"])["batch"].transform(
            "max"
        )  # zero‐based index of last batch
    ...
    fig, axes = plt.subplots(
        1,
        len(plot_loss_cols),
        figsize=(panel_width * len(plot_loss_cols), panel_width),
        sharex=True,
    )

    for ax, col in zip(axes, plot_loss_cols):
        sns.lineplot(data=loss_df, x="x", y=col, hue="type", style="split", ax=ax)
        ax.set_title(col)
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Loss")
