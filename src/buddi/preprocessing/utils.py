...
def get_true_proportions(
    in_adata: AnnData, cell_type_col: str, cell_order: Optional[Iterable[str]] = None
) -> pd.DataFrame:
    """
    ...
    if cell_order is None:
        ...
        if all([ctype in cell_order for ctype in in_adata.obs[cell_type_col].unique()]):
            ...
            raise ValueError(
                "cell_order must contain all unique cell types in the input data"
            )
    else:
...
def subset_adata_by_cell_type(
    ...
    for cell_type in cell_order:
        ...
        if cell_df[cell_type].shape[0] == 0:
            warnings.warn(
                f"Cell type '{cell_type}' not found in the input data. "
                "Creating an empty AnnData placeholder.\n"
...
def generate_random_similar_props(
    num_samp: int, props_df: Union[pd.DataFrame, pd.Series], min_corr: float = 0.8
) -> pd.DataFrame:
    """
...
def generate_single_celltype_dominant_props(
    ...
    for dominant_idx, cell_type in enumerate(cell_order):
        ...
        cell_dominance_metadata.extend([cell_type] * num_samp)
...
def generate_prop_from_counts(count_df: pd.DataFrame) -> pd.DataFrame:
    """
...
def generate_counts_from_props(
    ...
    if num_cells is None:
        ...
        if len(num_cells) != len(prop_df):
            raise ValueError(
                "If specified as a list, num_cells must have "
...
def generate_true_counts(
    in_adata: AnnData, num_cells: int, cell_type_col: str
) -> pd.DataFrame:
    """
...
def get_cell_type_sum(cell_adata: AnnData, num_cells: int) -> np.array:
    """
    ...
    sampled_cells = cell_adata[
        np.random.choice(cell_adata.shape[0], num_cells, replace=True)
    ]
