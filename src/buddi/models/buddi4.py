...
def build_buddi4(
    ...
    z_samp_type_input = tf.keras.Input(
        shape=z_samp_type.shape[1:], name="z_samp_type_input"
    )
    ...
    classifier_branch_label = build_latent_space_classifier(
        inputs=z_label_input, num_classes=n_labels, representation_name="label"
    )
    ...
    classifier_branch_stim = build_latent_space_classifier(
        inputs=z_stim_input, num_classes=n_stims, representation_name="stim"
    )
    ...
    supervised_decoder_input = tf.keras.Input(
        shape=supervised_decoder_concat.shape[1:], name="supervised_decoder_input"
    )

    unsupervised_decoder_concat = Concatenate(name="unsupervised_ls_concat")(
        [Y_hat, z_label, z_stim, z_samp_type, z_slack]
    )
    unsupervised_decoder_input = tf.keras.Input(
        shape=unsupervised_decoder_concat.shape[1:], name="unsupervised_decoder_input"
    )
    ...
    reconstr_loss_fn = reconstr_loss_generator(
        weight=1.0, reconstr_loss_fn=reconstr_loss_fn, reduction="sum"
    )
    ...
    shared_buddi_pred_losses = [
        classifier_loss_fn_label,
        classifier_loss_fn_stim,
        classifier_loss_fn_samp_type,
    ...
    shared_buddi_pred_loss_weights = [alpha_label, alpha_stim, alpha_samp_type]

    supervised_buddi_inputs = [X, Y]
    supervised_buddi_outputs = (
        [X_hat_supervised] + shared_buddi_z_params + shared_buddi_pred_outputs
    )
    ...
    unsupervised_buddi_ouputs = (
        [X_hat_unsupervised] + shared_buddi_z_params + shared_buddi_pred_outputs
    )
    unsupervised_buddi_pred_losses = shared_buddi_pred_losses + [
        unsupervised_dummy_loss_fn
    ]  # dummy loss item here that returns 0
...
def fit_buddi4(
    ...
    _ds_supervised = dataset_supervised.shuffle(
        num_sup_samples, reshuffle_each_iteration=shuffle_every_epoch
    ).batch(batch_size)
    _ds_unsupervised = dataset_unsupervised.shuffle(
        num_unsup_samples, reshuffle_each_iteration=shuffle_every_epoch
    ).batch(batch_size)

    n_batches = np.ceil(max(num_unsup_samples, num_sup_samples) / batch_size).astype(
        int
    )
    ...
    for epoch in range(epochs):
        ...
        for _, sup_batch in enumerate(
            tqdm(_ds_supervised, total=n_batches, desc=epoch_str)
        ):
        ...
        unsup_loss_df = pd.concat(
            [unsup_loss_df, unsup_batch_loss_df], ignore_index=True
        )

    print("Training complete!")

    sup_loss_df.columns = [
        col if i != 1 else "X_reconstruction_loss"
        for i, col in enumerate(sup_loss_df.columns)
    ]
    unsup_loss_df.columns = [
        col if i != 1 else "X_reconstruction_loss"
        for i, col in enumerate(unsup_loss_df.columns)
    ]
