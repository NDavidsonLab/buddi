...
def build_encoder_branch(
    ...
    :param representation_name: Name of the representation that this encoder should learn
        This representation will be enforced outside of this encoder branch in a supervised fashion
    ...
    hidden = Dense(
        hidden_dim, activation=activation, name=encoder_name + "_hidden_layer"
    )(inputs)
...
def build_prop_estimator(
    ...
    for i, hidden_dim in enumerate(hidden_dims):
        x = Dense(
            ...
            name=estimator_name + f"_hidden_layer_{i + 1}",
        )(x)

    output = Dense(
        num_classes, activation=output_activation, name=estimator_name + "_output_layer"
    )(x)
...
def build_latent_space_classifier(
    ...
    output = Dense(
        num_classes, activation=output_activation, name=classifier_name + "_output"
    )(activated_inputs)
...
def _build_decoder_layers(
    ...
    output_layer = Dense(
        output_dim, activation=output_activation, name=decoder_name + "_output_layer"
    )
...
def build_semi_supervised_decoder(
    ...
    supervised_decoder = Model(
        inputs_supervised, supervised_output, name=f"supervised_{output_name}_decoder"
    )
...
def build_decoder_branch(
    ...
    sup_model = Model(
        inputs=sup_inputs, outputs=sup_out, name=f"supervised_{output_name}_decoder"
    )
    ...
    for i, inp in enumerate(sup_inputs):
        ...
        shared_inputs.append(Input(shape=shape, name=f"shared_{output_name}_input_{i}"))
    shared_concat = Concatenate(name=f"decoder_{output_name}_shared_concat")(
        shared_inputs
    )
    x = shared_concat
    for layer in shared_hidden_layers:
    ...
    shared_model = Model(
        inputs=shared_inputs, outputs=shared_out, name=f"shared_{output_name}_decoder"
    )
