...
if not DATA_PATH.exists():
    raise FileNotFoundError(
        f"Data path {DATA_PATH} does not exist. Please add the example data to the examples/example_data/ directory."
    )
...
# %%
train_data = joblib.load(DATA_PATH / "buddi3_data.pkl")
...
# %%
(X_kp, y_kp, sample_id_kp, samp_type_kp, meta_kp) = (
    train_data.reset_query()
    .query(isTraining="Train", samp_type="sc_ref")
    .get(("X", "y", SAMPLE_ID_COL, TECH_COL, "meta"))
)

(X_unkp, y_unkp, sample_id_unkp, samp_type_unkp, meta_unkp) = (
    train_data.reset_query()
...
ds_unsup_batch_target_x, _, _, _, ds_unsup_batch_label, ds_unsup_batch_samp_type, _ = (
...
obj.set_encoder_loss(
    branch_name=[TECH_COL],
    fn=kl_loss,
    weight=BETA,
)
obj.set_encoder_loss(
    branch_name=[SAMPLE_ID_COL],
    fn=kl_loss,
    weight=BETA_SAMPLE,
)
obj.set_encoder_loss(
...
obj.set_predictor_loss(
    branch_name=[SAMPLE_ID_COL, TECH_COL],
...
if train_data.reset_query().query(isTraining="Test", samp_type="sc_ref").__len__() > 0:
    (test_X_kp, test_y_kp, test_sample_id_kp, test_samp_type_kp) = train_data.get(
        ("X", "y", SAMPLE_ID_COL, TECH_COL)
    )
...
if train_data.reset_query().query(isTraining="Test", samp_type="bulk").__len__() > 0:
    (test_X_unkp, test_y_unkp, test_sample_id_unkp, test_samp_type_unkp) = (
        train_data.get(("X", "y", SAMPLE_ID_COL, TECH_COL))
    )
...
# %% [markdown]
# ### Visualize latent space of Train
...
# %% [markdown]
# ### Visualize latent space don't filter by split
